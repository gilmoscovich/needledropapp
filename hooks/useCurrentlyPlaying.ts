...
// Fetches currently playing track from Spotify, falling back to recently played.
// For recently-played tracks, resolves the correct trackIndex via a full album fetch.

import { useState, useCallback } from 'react';
import { msToTimestamp, getAlbum } from '@/services/spotify';
...
export interface CurrentTrackInfo {
  ...
  trackName:      string;
  trackNum:       number;
  trackIndex:     number;      // 0-based, fully resolved
  albumId:        string;
  albumName:      string;
  albumUri:       string;
  artist:         string;
  ...
  needsFullAlbum: boolean;     // always false after resolution
}
 ...
 * recently-played returns a simplified album without tracks.items,
