# NeedleDrop — Frontend Phase 1: Root Layout & Login Screen

## Goal
Set up Expo Router's root layout (fonts, auth gate), the redirect
index, and the Login screen. After this phase the app has a working,
beautifully styled login flow.

---

## Prerequisites
All five backend phases complete — auth, Spotify API, bookmarks,
playback, and toast hooks all exist.

---

## Step 1 — Update `tsconfig.json`

Add the path alias so `@/` works across every file:
...
The root layout loads fonts, gates the auth state, and wraps
everything in the navigation stack.
...
Redirect users to the right place based on auth state:
...
## Step 4 — Create `app/login.tsx`

The full login screen. Vinyl circle animation, wordmark, tagline,
and the Spotify login button.
...
Run `npx expo start`, open on iPhone.
You should see:
- Dark background with a large spinning vinyl record
- "NEEDLE DROP" wordmark + tagline fades in
- "Continue with Spotify" button at the bottom
- Tapping login opens Spotify OAuth
- After auth: lands on `/library` (next phase)

---

## Definition of done

- [ ] Fonts load (Manrope at 4 weights)
- [ ] `app/index.tsx` redirects to `/login` if unauthenticated
- [ ] `app/index.tsx` redirects to `/(tabs)/library` if authenticated
- [ ] Vinyl record spins slowly
- [ ] Wordmark + tagline fades in on mount
- [ ] Login button triggers Spotify OAuth
- [ ] After login: redirects to library (may show empty until Phase 2)
